...
import { Project, OutsideProject } from '../types/content';
import { useLanguage } from '../contexts/LanguageContext';
import { Calendar } from 'lucide-react';

export default function OutsideProjects() {
  ...
  if (loading) return <div className="py-20 text-center">{t('outsideProjects.loading')}</div>;
  if (error) return <div className="py-20 text-center">{t('outsideProjects.error')}</div>;
  if (!content?.outsideProjects) return null;

  return (
    <section id="outside-projects" className="py-20 bg-black">
      <div className="max-w-[1400px] mx-auto px-6 sm:px-8 md:px-12 lg:px-16">
        <motion.div 
          ...
          <h1 className="text-3xl min-[430px]:text-4xl md:text-5xl font-bold text-stone-200">
            {t('outsideProjects.title')}
          </h1>
          <p className="text-stone-200/70 text-sm min-[430px]:text-base max-w-lg md:max-w-3xl">
            {t('outsideProjects.description')}
          </p>
        </motion.div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {content.outsideProjects.map((project: OutsideProject) => (
            <motion.div
              key={project.title}
              variants={itemVariants}
              initial="hidden"
              ...
              <div className="p-6 flex flex-col flex-grow">
                ...
                <div className="space-y-4 mt-6 pt-6 border-t border-dark-700">
                  ...
                  <div className="flex items-center gap-2 text-sm text-gray-400">
                    <Calendar size={14} />
                    <span>{t('outsideProjects.date')}{project.projectDate}</span>
                  </div>
                </div>
              </div>
            </motion.div>
